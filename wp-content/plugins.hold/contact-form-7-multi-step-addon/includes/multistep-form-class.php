...
class Multistep_Form {
    ...
    public function add_shortcodes() {
        ...
        wpcf7_add_form_tag( 'step_start', array($this, 'step_start_shortcode') );
        wpcf7_add_form_tag( 'step_finish', array($this, 'step_finish_shortcode') );
    ...
    public function validate_fields_ajax() {
	    ...
	    foreach ($tags as $field ) {
		    $validate_schema_field[] = $field->name;
	    }
    ...
    private function prepare_invalid_form_fields ($result){
        ...
        foreach ((array)$result->get_invalid_fields() as $name => $field) {
            $invalid_fields[] = array(
                'into' => '.wpcf7-form-control-wrap[data-name="'. sanitize_html_class($name) .'"]',
